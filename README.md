...
### ETL Pipeline with Windowing and Metrics
```typescript
...
const pipeline = createPipeline<Meta, {}, typeof macros>(
  macros,
  () => ({})  // Base context factory
)

// 3. Create a step with metadata
const fetchWeather: Step<Meta, {}, typeof macros, Weather> = {
  name: 'fetch-weather',
  meta: {
    env: ['WEATHER_API_KEY'],
...
| `sinkMacro` | `sink: boolean` | `ctx.emit()` function | Event collection |
...
**Ready to eliminate boilerplate?** Check out the [examples](./examples) or read the [core](./core.ts) (under 80 lines!).