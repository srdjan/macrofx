...
const authMacro = {
  ...
  onError: (_b: Base, _m: Meta, err: unknown) =>
    String(err).includes("unauthorized") ? { ok: false, reason: "401" } : undefined,
} as const;
...
const txMacro = {
  ...
  after: (
    ctx: Base & { tx: { committed: boolean } },
    _m: Meta,
    result: { ok: boolean; by: string; mode: "ro" | "rw" },
  ) => {
    ctx.tx.committed = true;
    return { ...result, txCommitted: true as const };
  },
} as const;
